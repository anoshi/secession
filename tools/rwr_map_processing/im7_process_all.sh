...
if [[ -z $terrain_layers ]]; then
    if [[ ! -z $alpha_layers ]]; then
        ...
        read -p "No terrain layers specified. Allow script to proceed with best-guess defaults [YES/no]? " default_terrain
        case ${default_terrain:0:1} in
            n|N|No|NO)
