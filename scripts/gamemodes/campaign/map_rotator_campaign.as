...
class MapRotatorCampaign : MapRotatorInvasion {
	...
	protected bool checkForFinalBattleUnlock() {
		...
		for (uint i = 0; i < m_factionConfigs.size(); ++i) {
			...
			if (!hasBeenAllocatedForFinalBattle(factionConfigIndex) && isFactionCompletelyDefeated(factionConfigIndex)) {
	...
	protected void handleHitboxEvent(const XmlElement@ event) {
		...
		// vanilla took great delight in stealing all of my hitbox events.
		// allow change map to trigger only when hitbox id does not contain 'hitbox_trigger'
		if (event.getStringAttribute("instance_type") == "character" &&
			event.getIntAttribute("instance_id") == m_playerCharacterId &&
			!startsWith(event.getStringAttribute("hitbox_id"), "hitbox_trigger")) {
