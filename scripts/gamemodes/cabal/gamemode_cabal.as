...
// secession helpers
#include "secession_helpers.as" // rename to cabal_helpers.as

// cabal trackers
#include "dummy_vehicle_handler.as"
//#include "item_drop_handler.as"
#include "character_lifecycle_handler.as"
...
class Cabal : GameMode {
	...
	void postBeginMatch() {
		...
		//addTracker(ItemDropHandler(this)); // bugged: does not track when weapons are dropped on death, only carry items
		addTracker(CharacterLifecycleHandler(this)); // using this method instead to create a chance to drop an item near player after killing bad guys
