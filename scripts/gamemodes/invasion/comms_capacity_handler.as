...
class CommsCapacityHandler : Tracker {
	...
	protected void handleCommsChangeEvent(const XmlElement@ event) {
		...
		_log("CommsCapacityHandler, handleCommsChangeEvent, factionId=" + factionId + ", state=" + comms);
		if (!comms) {
	...
	protected void handleBaseOwnerChangeEvent(const XmlElement@ event) {
		...
		if (oldOwner >= 0 && !hasFactionComms(m_metagame, oldOwner)) {
			_log("CommsCapacityHandler, handleBaseOwnerChangeEvent, previous_owner_id=" + oldOwner + ", no comms");
