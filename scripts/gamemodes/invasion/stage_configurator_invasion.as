...
class StageConfiguratorInvasion : StageConfigurator {
    ...
    stage.m_fogRange = 50.0;     
		...
		stage.m_playerAiCompensation = 6;   // 8 in 0.99.4
	...
	protected Stage@ setupStage3() {
		...
		stage.m_soldierCapacityVariance = 0.31;
	...
	protected Stage@ setupStage4() {
		...
		stage.m_playerAiCompensation = 4; // instead of 8      
		stage.m_defenseWinTime = 400;
		...
		{
			Faction f(getFactionConfigs()[2], createCommanderAiCommand(2, 0.1, 0.1));
			stage.m_factions.insertLast(f);
		}
		{
			Faction f(getFactionConfigs()[1], createCommanderAiCommand(1, 0.1, 0.1));
			stage.m_factions.insertLast(f);
		}
		{
			// neutral
	...
	protected Stage@ setupStage5() {
		...
		stage.m_playerAiCompensation = 2; // instead of 8
...

		{
			...
			f.m_overCapacity = 6;
			f.m_bases = 1;
			f.m_capacityMultiplier = 0.81; // by default, green has 120%, this will make it a bit less so in map1
			stage.m_factions.insertLast(f);
		}
		{
			Faction f(getFactionConfigs()[1], createCommanderAiCommand(1));
	...
	protected Stage@ setupStage7() {
...

		{
			...
			Faction f(getFactionConfigs()[0], createFellowCommanderAiCommand(0, 0.3, 0.12));
			f.m_overCapacity = 0;
			f.m_capacityOffset = 0; // handled in the vehicle files (0.95)
			f.m_capacityMultiplier = 0.9;
			f.m_bases = 1;
			stage.m_factions.insertLast(f);
		}
		{
			Faction f(getFactionConfigs()[2], createCommanderAiCommand(1));
			f.m_overCapacity = 70;
			f.m_capacityOffset = 10; // was set to 20, now 10 (0.95)
			stage.m_factions.insertLast(f);
		}
	...
	protected Stage@ setupStage8() {
		...
		stage.addTracker(Overtime(m_metagame, 0));
		stage.addTracker(Spawner(m_metagame, 1, Vector3(255,0,344)));
		stage.addTracker(Spawner(m_metagame, 1, Vector3(288,0,359)));
		stage.addTracker(Spawner(m_metagame, 1, Vector3(319,0,389)));
		stage.addTracker(Spawner(m_metagame, 1, Vector3(289,0,410)));
		stage.addTracker(Spawner(m_metagame, 1, Vector3(311,0,420)));

		stage.m_maxSoldiers = 33 * 3;
		stage.m_playerAiCompensation = 4; // instead of 8 (in 1.40)    
		stage.m_soldierCapacityModel = "constant";
		stage.m_defenseWinTime = 600.0;
		...
		{
			Faction f(getFactionConfigs()[1], createCommanderAiCommand(1, 0.2, 0.1));
			f.m_capacityMultiplier = 1.32;
			f.m_overCapacity = 50;
			stage.m_factions.insertLast(f);
		}
		{
	...
	protected Stage@ setupStage9() {
		...
		stage.m_maxSoldiers = 220;
		stage.m_soldierCapacityVariance = 0.4;
		stage.m_playerAiCompensation = 5; // 4 in 0.99.4

		stage.addTracker(PeacefulLastBase(m_metagame, 0));

		{
			Faction f(getFactionConfigs()[0], createFellowCommanderAiCommand(0, 0.2, 0.2));
			f.m_overCapacity = 0;
			f.m_capacityMultiplier = 0.9;
			stage.m_factions.insertLast(f);
		}
	...
	protected Stage@ setupStage10() {
		...
		{ 				
			Faction f(getFactionConfigs()[0], createFellowCommanderAiCommand(0, 0.1, 0.2));
			f.m_overCapacity = 0;
		...
		{
			Faction f(getFactionConfigs()[2], createCommanderAiCommand(1, 0.6, 0.2));
			f.m_overCapacity = 50;
    ...
    stage.m_fogRange = 50.0; 
		...
		stage.m_playerAiCompensation = 5;          // instead of 2 in 1.50
...
//		stage.addTracker(Spawner(m_metagame, 1, Vector3(658,10,374), 10));        // vulcan slot filler
	...
	protected Stage@ setupFinalStage1() {
		...
		stage.m_playerAiCompensation = 4;    // 3 in 0.99.4
