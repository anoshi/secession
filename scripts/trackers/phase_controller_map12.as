...
class Phase : Tracker {
	...
	// --------------------------------------------
	Phase(Metagame@ metagame, PhaseControllerMap12@ controller) {
		@m_metagame = @metagame;
...
class Phase1 : Phase {
	...
	// --------------------------------------------
	Phase1(Metagame@ metagame, PhaseControllerMap12@ controller) {
...
class Phase2 : Phase {
	// --------------------------------------------
	Phase2(Metagame@ metagame, PhaseControllerMap12@ controller) {
		super(metagame, controller);
	}
	...
	void start() {
		...
		m_metagame.getComms().send(
			...
			"  <faction capacity_multiplier='0.8' />" + 
			"  <faction capacity_multiplier='0.5' />" + 
			"  <faction />" + 
			"</command>");
		...
		if (!hasEnded()) {
			...
			m_metagame.getTaskSequencer().add(AnnounceTask(m_metagame, 5.0, 0, textKeyPrefix +"part 4"));
			m_metagame.getTaskSequencer().add(AnnounceTask(m_metagame, 5.0, 0, textKeyPrefix +"part 5"));
		}
	}
...
class Phase3 : Phase {
	// --------------------------------------------
	Phase3(Metagame@ metagame, PhaseControllerMap12@ controller) {
		super(metagame, controller);
	}
	...
	void start() {
		...
		m_metagame.getComms().send(
			"<command class='change_game_settings'>" + 
			"  <faction capacity_multiplier='0.4' />" + 
			"  <faction capacity_multiplier='0.45' />" + 
			"  <faction />" + 
			"</command>");
...
class Phase4 : Phase {
	...
	Phase4(Metagame@ metagame, PhaseControllerMap12@ controller) {
		super(metagame, controller);
	...
	void start() {
		...
		m_metagame.getComms().send(
			"<command class='change_game_settings'>" +
			"  <faction capacity_multiplier='0.3' />" + 
			"  <faction capacity_multiplier='0.6' />" + 
			"  <faction />" +
			"</command>");
		...
		if (!m_continueMode) {
			...
			m_metagame.addTracker(Spawner(m_metagame, 1, Vector3(638,0,522)));
			m_metagame.addTracker(Spawner(m_metagame, 1, Vector3(686,0,521)));
			m_metagame.addTracker(Spawner(m_metagame, 1, Vector3(707,0,460)));
			m_metagame.addTracker(Spawner(m_metagame, 1, Vector3(655,0,435)));
			m_metagame.addTracker(Spawner(m_metagame, 1, Vector3(699,0,391)));
			m_metagame.addTracker(Spawner(m_metagame, 1, Vector3(617,0,408)));
...
class PhaseControllerMap12 : PhaseController {
	...
	// --------------------------------------------
	PhaseControllerMap12(Metagame@ metagame) {
		@m_metagame = @metagame;
		m_started = false;
