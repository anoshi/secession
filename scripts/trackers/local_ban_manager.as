...
class LocalBanManager : Tracker {
	...
	void handleTempBan(string message, int senderId, bool id = false) {
		...
		if (player !is null) {
			...
			// ban
			addBannedPlayer(playerName, DEFAULT_BAN_TIME);
			sendPrivateMessage(m_metagame, senderId, "banning " + playerName + " for " + (DEFAULT_BAN_TIME/60) + " minutes");
			// kick
			kickPlayer(playerId);
		} else {
	...
	protected void handlePlayerConnectEvent(const XmlElement@ event) {
		...
		if (player !is null) {
			string playerName = player.getStringAttribute("name");
			if (isBanned(playerName.toLowerCase())){
