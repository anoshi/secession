...
class Spawner : Tracker {
	...
	// --------------------------------------------
	Spawner(Metagame@ metagame, int factionId, const Vector3@ position, int count = 5) {
		@m_metagame = @metagame;
	...
	void start() {
		...
		for (int i = 0; i < m_spawnCount; ++i) {
			string command = "<command class='create_instance' faction_id='" + m_factionId + "' position='" + m_position.toString() + "' instance_class='character' instance_key='default' />";
